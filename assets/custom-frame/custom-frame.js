...
class CustomFrame {
	...
	create() {
		...
		that.window.addEventListener('resize', () => {
			// Electron doesn't trigger 'restore' event in some cases
			if (that.window.localStorage.customFrameState == 'maximized' && !options.win.isMaximized()) {
				onRestore()
			}
		})
		...
		options.win.on('restore', onRestore);
