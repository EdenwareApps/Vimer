...
class BridgeServer extends Events {
    constructor(opts){
        super()
        this.ua = 'Vimer '+ global.MANIFEST.version
        if(!global.cordova) {
            ...
            if(!this.lang){
                ...
                this.lang.load().then(() => {
                    this.lang.availableLocalesMap().then(ret => {
                        this.emit('available-languages', ret, this.lang.locale)
                    }).catch(console.error)
                }).catch(console.error)
            }
        })
        this.on('config-set', (key, data) => this.config.set(key, data))
        this.config.on('change', async (keys, data) => {
            this.emit('config', data, keys)
            if(keys.includes('locale') && data.locale) {
                let texts = await this.lang.loadLanguage(data.locale).catch(console.error)
                if(texts){
                    this.lang.locale = data.locale
                    this.lang.applyTexts(texts)
                }
                this.emit('lang', this.lang.getTexts(), data.locale)
            }
        })
    }
    on(...args){
